...
public class ScheduledSendService {
    ...
    public ScheduledSendService(ScheduledSendRepository scheduledSendRepository,
    ...
    public ScheduledSend save(String username, ScheduledSendRequest request) {
        ScheduledSend schedule = scheduledSendRepository.findByUsername(username)
                .orElseGet(() -> {
        ...
        String channel = request.getChannel() == null || request.getChannel().isBlank()
                ? "WHATSAPP" : request.getChannel().toUpperCase();
        ...
        // Only strip to digits for a phone number - an email address needs its punctuation kept.
        schedule.setToNumber("EMAIL".equals(channel)
                ? request.getToNumber().trim()
    ...
    public void checkAndSend() {
        ...
        for (ScheduledSend schedule : due) {
            boolean isDue = schedule.getDayOfWeek() == now.getDayOfWeek()
                    && schedule.getHour() == now.getHour()
            ...
            if (isDue && !alreadySentToday) {
                ...
                if (!items.isEmpty()) {
                    if ("EMAIL".equals(schedule.getChannel())) {
                        emailService.sendGroceryList(schedule.getUsername(), schedule.getToNumber(),
                                today, items, true);
                    } else {
                        whatsAppService.sendGroceryList(schedule.getUsername(), schedule.getToNumber(),
                                today, items, true);
                    }
                }
