(function() {
    ...
    async function loadSchedule() {
        try {
            ...
            if (schedule) {
                ...
                // Restore the grocery date used for this schedule
                if (schedule.groceryDateTimeStamp) {
                    document.getElementById('list-date').value =
                        schedule.groceryDateTimeStamp.substring(0, 10);
                }
            }
        } catch (err) {
    ...
    document.getElementById('save-schedule-btn').addEventListener('click', async () => {
        ...
        const toNumber = channel === 'EMAIL'
            ? rawRecipient
            : rawRecipient.replace(/[^0-9]/g, '');

        // Selected grocery list date
        const selectedDate = document.getElementById('list-date').value;

        if (!selectedDate) {
            scheduleError.textContent = 'Select a grocery list date.';
            return;
        }

        const groceryDateTimeStamp = selectedDate + "T00:00:00";
        ...
        if (channel === 'EMAIL') {
            if (!toNumber.includes('@')) {
                scheduleError.textContent = 'Enter a valid email address.';
                return;
            }
        } else {
        ...
        try {
            ...
            await apiFetch('/api/whatsapp/schedule', {
                ...
                body: JSON.stringify({
                    enabled,
                    dayOfWeek: scheduleDaySelect.value,
                    hour,
                    minute,
                    channel,
                    toNumber,
                    groceryDateTimeStamp
                })
            });
            ...
            if (err.message !== 'Unauthorized') {
                scheduleError.textContent =
                    err.message || 'Could not save schedule';
            }
        }
    });
    // ---------- History ----------
